...
export class OfferService {
 ...
 PostNewOffer(name:string,dueto:string,companyId:string) {//company posts an offer
    return this.http
      .post(
        'http://localhost:3000/api/Offer',{name,dueto,companyId}
      )
  }
...
approveForFreeLancer(OfferID:string,freelancerID:string,CompanyID){
  return this.http
  .post('http://localhost:3000/api/freeOffer/apply',{OfferID,freelancerID,CompanyID})
}
